...
public interface DistributeLockDAO extends JpaRepository<DistributeLock, Integer> {
    ...
    @Query(value =
            "select count(id) from distributed_lock dl " +
                    "where dl.service_key = :serviceKey and dl.lock_key = :lockKey and dl.owner = :owner " +
                    "and dl.expire_seconds > TIMESTAMPDIFF(SECOND, dl.renewal_time, NOW())", nativeQuery = true)
    ...
    @Query(value =
            "select dl.id, dl.service_key, dl.lock_key, dl.owner, dl.expire_seconds, dl.create_time, dl.renewal_time " +
                    ...
                    "where dl.expire_seconds < TIMESTAMPDIFF(SECOND, dl.renewal_time, NOW())", nativeQuery = true)
    ...
    Optional<DistributeLock> findTimeoutLock(@Param("serviceKey") String serviceKey, @Param("lockKey") String lockKey, @Param("owner") String owner);

    Optional<DistributeLock> findByServiceKeyAndLockKeyAndOwner(String serviceKey, String lockKey, String owner);
